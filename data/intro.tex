...
Many of the most consequential computational challenges in science and engineering can
be naturally expressed as combinatorial optimization (CO) problems on graphs. From
...
systems, graph-based CO problems are pervasive. These problems are also notoriously
difficult: among the 21 NP-complete problems introduced by
Karp~\cite{karp2009reducibility}, 10 are decision versions of graph optimization
problems, and many of the remaining ones can be naturally reformulated on graphs. Over
...
As graph data continues to grow in scale, the practical applicability of even the best
solvers has become a pressing concern. Modern real-world graphs routinely contain
...
fundamental scalability bottleneck. A natural strategy to address this bottleneck is
pruning: constructing a much smaller ground set
$\mathcal{U}' \subseteq \mathcal{U}$ with $|\mathcal{U}'| \ll |\mathcal{U}|$ such that
solving the reduced instance preserves near-optimal solution quality. However, existing
