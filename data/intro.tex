...
systems, graph-based CO problems are pervasive. These problems are, in general, computationally intractable: among the 21 NP-complete
problems introduced by Karp~\cite{karp2009reducibility}, 10 are decision versions of
graph optimization problems, and many of the remaining ones can be naturally reformulated
on graphs. Over
...
practical progress on problems that are intractable to solve exactly.
...
marketing, a company may have the budget to promote only a handful of products while
aiming to maximize overall customer reach~\cite{kempe2003maximizing}. In these settings,
...
$\mathcal{U}' \subseteq \mathcal{U}$ with $|\mathcal{U}'| \ll |\mathcal{U}|$. A solver
can then operate on the reduced instance $\mathcal{U}'$ instead of $\mathcal{U}$, ideally
with little or no loss in solution quality. Formally, a CO problem can be expressed as a
tuple $(S, \Omega, f)$, where $S$ denotes the search space over discrete decision
variables, $\Omega$ represents the set of constraints that define feasibility, and
$f: S \to \mathbb{R}$ is the objective function to be optimized. Given a ground set
$\mathcal{U}$ and an exact solver $\mathcal{OPT}$, the optimal solution is
$X^\star = \mathcal{OPT}(\mathcal{U}) = \arg\max_{X \subseteq \mathcal{U}, \; X \text{
satisfies } \Omega} f(X)$. The pruning problem seeks to construct a much smaller ground
set $\mathcal{U}'$ with $|\mathcal{U}'| \ll |\mathcal{U}|$, such that solving the reduced
instance yields nearly the same objective value:
$f(\mathcal{OPT}(\mathcal{U}')) / f(\mathcal{OPT}(\mathcal{U})) = 1 - \epsilon$ for a
small approximation factor $\epsilon \geq 0$. However, existing
pruning approaches are limited by poor scalability of sequential classical
methods~\cite{zhou2017scaling}, reliance on hand-crafted, problem-specific features in
learning-based methods~\cite{manchanda2020gcomb, ireland2022lense,
tian2024combhelper}, and a complete absence of theoretical guarantees on pruning quality.
...
graph-based combinatorial optimization, aiming to make solvers faster, more general, and
more practical for large-scale graph instances.

Prior to our work, all existing pruning methods were heuristics with no formal
...
QuickPrune~\cite{nath2025quickprune}, a lightweight pruning algorithm for constrained
