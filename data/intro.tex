...
networks~\citep{kempe2003maximizing} to designing efficient transportation
routes~\citep{sun2021generalization} and allocating resources in telecommunication
...
problems introduced by Karp~\citep{karp2009reducibility}, 10 are decision versions of
...
time~\citep{nemhauser1978analysis, kempe2003maximizing, tang2015influence}, enabling
practical solutions to problems that are intractable to solve exactly.
...
aiming to maximize overall information spread~\citep{kempe2003maximizing}. In these settings,
...
fundamental scalability bottleneck. A natural strategy to address this bottleneck is pruning: given the full ground set $\mathcal{U}$, the goal is to identify and discard elements that are unlikely to contribute to a high-quality solution, producing a much smaller subset $\mathcal{U}' \subseteq \mathcal{U}$ with $|\mathcal{U}'| \ll |\mathcal{U}|$. A solver can then operate on $\mathcal{U}'$ instead of $\mathcal{U}$, ideally with little or no loss in solution quality. Formally, a CO problem instance is a tuple
$(\mathcal{U}, f, \Omega)$, where $\mathcal{U}$ is the ground set of candidate
elements, $f: 2^{\mathcal{U}} \to \mathbb{R}_{\geq 0}$ is the objective function,
and $\Omega$ is the feasibility constraint. Given a ground set $\mathcal{U}$ and a
solver $\mathcal{OPT}$, the optimal solution is
%
\begin{equation}
...
\end{equation}
%
for a small approximation factor $\epsilon \geq 0$. However, existing pruning approaches suffer from several limitations. Classical methods
such as~\citet{zhou2017scaling} process elements sequentially, limiting their scalability
to large instances. Problem-specific methods are restricted to a single CO problem, such
as maximum clique enumeration~\citep{lauri2019fine, grassia2019learning, lauri2023learning}
or the traveling salesman problem~\citep{fitzpatrick2021learning, sun2021generalization},
and do not transfer across problems. Learning-based
methods~\citep{manchanda2020gcomb, ireland2022lense, tian2024combhelper} rely on
hand-crafted, problem-specific features that require significant domain expertise.
Finally, no existing method provides formal guarantees on the quality of the pruned
ground set.
Altogether, these limitations motivate a focused research effort on developing pruning
strategies that are scalable, general, and principled. The overarching goal of this
dissertation is to address this challenge by developing a suite of pruning methods for
...
performance guarantees. It was unknown whether it was even possible to efficiently produce
...
QuickPrune~\citep{pmlr-v258-nath25a}, a lightweight pruning algorithm for constrained
...
prunes the ground set and outperforms state-of-the-art classical and learning-based
algorithms for pruning.
...
is \#P-hard~\citep{chen2010scalable}.
A complementary approach to classical pruning is to learn to prune from data, enabling
faster inference without manual feature design. However, existing learning-based methods
demand problem-specific domain knowledge for feature engineering, making them difficult
to apply to new CO problems. To address this limitation, we present
Hierarchical DeepPruner (H-DeepPruner)~\citep{nath2025hdeeppruner}, a
...
interdependencies among elements. As a proof of concept, our experiments demonstrate
that H-DeepPruner achieves competitive performance with existing learning-based pruning
methods across multiple graph CO problems, showing that effective pruning is possible
without domain-specific knowledge or hand-crafted features. However,
H-DeepPruner is designed for size-constrained problems and does not directly extend to
...
present LLM2Prune~\citep{nath2026llmprune}, a framework that leverages large language
...
engineering. LLM2Prune automatically discovers non-trivial features that would typically
...
\textbf{Contributions.} In summary, \emph{This dissertation presents a coherent framework for scaling combinatorial optimization
on graphs through provable guarantees, learned pruning, and automated feature discovery.} QuickPrune establishes the theoretical foundations, proving that pruning with
formal value-retention bounds is achievable under natural assumptions. Hierarchical DeepPruner demonstrates that learning-based methods can achieve effective empirical
...
The remainder of this dissertation is organized as follows. Chapter~\ref{chap:relatedwork} reviews related work
on pruning methods for combinatorial optimization. Chapters~\ref{chap:quickprune}, \ref{chap:hdeeppruner}, and \ref{chap:llm2prune} present
QuickPrune, Hierarchical DeepPruner, and LLM2Prune in detail, respectively. Chapter~\ref{chap:conclusion}
