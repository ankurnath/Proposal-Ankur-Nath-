...
(Chapter~4) demonstrates that learning-based pruning can be problem-agnostic, both
...
Three meta-prompts are used: a summary prompt that condenses the history into key points, a
feature-generation prompt that proposes new feature sets conditioned on the task description
and history summary, and a code-generation prompt that produces executable Python code for
...
\label{sec:llm2prune-remaining}

We have completed the proposed framework design, the ablation study justifying the beam
search strategy under both size and knapsack constraints, and a comprehensive comparative
evaluation under size constraints across three CO problems and eight real-world datasets.
The results demonstrate that LLM2Prune performs competitively with classical methods while
being orders of magnitude faster at inference, and consistently outperforms all existing
learning-based approaches. All results reported in this chapter were generated using GPT-4.1 as the backbone LLM.
The primary remaining work is conducting a full comparative evaluation under knapsack
constraints against all baselines across all datasets and applications. Unlike other
learning-based methods, LLM2Prune can naturally adapt to the knapsack constraint by simply
modifying the task description provided to the LLM, without requiring any changes to the
underlying framework or retraining the feature discovery pipeline.
