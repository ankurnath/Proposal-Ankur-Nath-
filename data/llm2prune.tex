...
\chapter{\uppercase {LLM2Prune: Using LLMs as Domain Experts for Search Space Reduction}} \label{chap:llm2prune}

While QuickPrune (Chapter~\ref{chap:quickprune}) provides theoretically grounded pruning and H-DeepPruner
(Chapter~\ref{chap:hdeeppruner}) demonstrates that learning-based pruning can be problem-agnostic, both
approaches have limitations: QuickPrune processes elements sequentially and does not
scale well to problems where marginal gain computation is \#P-hard, while H-DeepPruner
operates only under size constraints and provides no theoretical guarantees.
...
over graphs, and adapts easily to different constraints?} In this chapter, we present
LLM2Prune~\citep{nath2026llmprune}, a framework that uses large language models (LLMs) to automatically generate
...
Submodular Sparsification~\citep{zhou2017scaling} process elements sequentially, which
does not scale well to problems where marginal gain computation is \#P-hard. Among learning-based methods,
...
and problem-specific algorithmic choices. We address this bottleneck by
leveraging large language models to automate the feature discovery process. LLMs are
trained on large corpora that include graph theory, network science, and combinatorial
optimization literature, making them capable of proposing problem-relevant features from
a natural language task description alone~\citep{yang2023large, liu2024evolution,
han2024large, hollmann2023large}. We treat this as an empirical hypothesis:
rather than assuming LLM knowledge is complete, we use performance on held-out validation
graphs as the selection signal, so the beam search retains only features that demonstrably
improve pruning quality. This design is robust to LLM errors --- incorrect or
uninformative features are simply discarded based on performance. The discovered features
reported in Section~\ref{sec:llm2prune-features} provide empirical support: LLM2Prune
consistently identifies non-trivial, problem-grounded features without any human
intervention. However, LLM knowledge is not unlimited: when the task description is
ambiguous or the problem domain falls outside the LLM's training distribution, the
quality of proposed features may degrade. We discuss this limitation further in
Section~\ref{sec:llm2prune-remaining}.
...
\begin{figure}[t]
  ...
  \caption{(a) Two phases of LLM2Prune, illustrated with beam width $\beta = 2$ and
    ...
    to form child nodes. Reprinted from~\citep{nath2026llmprune}.}
...
\Big(\mathcal{F}_{\ell,j}, \mathcal{M}_{\ell,j}, \text{Imp}_{\ell,j}, \mathcal{F}^{\text{fail}}_{\ell,j}\Big),
\]
where $\pi(\ell,i)$ denotes the ancestor index at depth $\ell$ along the path from the
root to $n_{d,i}$, $\mathcal{M}_{\ell,j}$ is the performance metric, $\text{Imp}_{\ell,j}$ is the
...
First, the LLM summarizes the cumulative history $\mathcal{H}_{d,i}$ using the summary
meta-prompt $Z_{\text{summary}}$, which condenses the feedback into key points and
insights, keeping it compact for the LLM to reason over:
...
Then, using the feature-generation meta-prompt $Z_{\text{feature}}$ conditioned on the
task description and the history summary, the LLM proposes $\gamma$ child feature sets.
The prompt instructs the LLM to retain important features, remove redundant ones, and
propose new features derived from high-performing ones:
\begin{minted}[fontsize=\footnotesize, frame=single, breaklines]{python}
f'''You are an expert in graph neural networks and combinatorial optimization.
...
'''
\end{minted}
Formally, the child feature sets are sampled as:
\[
...
diverse outputs due to temperature sampling, broadening exploration of the feature space.

For each child node with its generated feature set $\mathcal{F}_{d+1,j}$, the LLM is
prompted with the code-generation meta-prompt $Z_{\text{code}}$ to produce executable
Python code for each proposed feature: \newpage
\begin{minted}[fontsize=\footnotesize, frame=single, breaklines]{python}
f'''The input is a weighted NetworkX graph G where each node has an attribute
weight and an integer variable budget is provided.

Feature name: {feature}
Feature definition: {definition}

"Write Python code for a function extract_feature(G) that computes this
feature for ALL nodes in G. The function must return a NumPy array with
one value per node, ordered to align with the iteration order of G.nodes()."
"Keep the implementation efficient; avoid expensive computations."
"DO NOT INCLUDE ANY EXPLANATIONS OR COMMENTS."
'''
\end{minted}
Each feature must return a NumPy array with one value per node. A timeout threshold $\tau$ is imposed to ensure computational efficiency: if
...
the contribution of each feature to the predictions of the classifier. These scores form the
...
$n_{d+1,j}$ using its performance metric $\mathcal{M}_{d+1,j}$ and constructs the next beam
...
\[
n^{\text{best}} = \arg\max_{n_{d,i} \in \mathcal{T}} \mathcal{M}_{d,i}.
\]
...
\FOR{$d = 0, 1, \dots, D_{\max} - 1$}
    ...
    \FOR{each $n_{d,i} \in B_d$}
        ...
        \FOR{$j = 1, \dots, \gamma$}
            ...
            \STATE Compute $\mathcal{M}_{d+1,j}$ and $\text{Imp}_{d+1,j}$; update $\mathcal{H}_{d+1,j}$
            \STATE $\mathcal{C}_{d+1} \leftarrow \mathcal{C}_{d+1} \cup \{n_{d+1,j}\}$
        \ENDFOR
    \ENDFOR
    \STATE $B_{d+1} \leftarrow \text{top-}\beta$ nodes in $\mathcal{C}_{d+1}$ by $\mathcal{M}$ \hfill \textit{// Beam Pruning}
\ENDFOR
...
LLM2Prune separates the feature discovery cost (a one-time beam search on synthetic
graphs) from the inference cost (a single GCN forward pass on each target instance).

\textbf{Beam Search.} At each depth $d \in \{0, \ldots, D_{\max}-1\}$, the beam
contains at most $\beta$ nodes. Each node is expanded into $\gamma$ children, yielding
at most $\beta\gamma$ candidate nodes per depth. For each candidate, the LLM is called
twice (once for summarization, once for feature generation) and once more per proposed
feature for code generation, giving $O(\gamma)$ calls per candidate. The total number
of LLM API calls across the entire search is therefore $O(D_{\max} \cdot \beta \cdot
\gamma)$. With the fixed hyperparameters $D_{\max} = 5$, $\beta = 3$, $\gamma = 2$,
this amounts to a constant number of API calls. For each of the $O(D_{\max} \cdot \beta
\cdot \gamma)$ candidate nodes, a two-layer GCN is trained on a synthetic graph with
$|V_{\text{syn}}|$ nodes and $|E_{\text{syn}}|$ edges for a fixed number of epochs, and
GNNExplainer~\citep{ying2019gnnexplainer} is run to extract feature-importance scores.
Both costs are $O(|V_{\text{syn}}| + |E_{\text{syn}}|)$ per training epoch and per
explainer optimization step, respectively. The total beam search cost is therefore
$O(D_{\max} \cdot \beta \cdot \gamma \cdot \text{steps} \cdot
(|V_{\text{syn}}| + |E_{\text{syn}}|))$,
where $\text{steps}$ subsumes both the GCN training epochs and the GNNExplainer
optimization steps. Crucially, the entire beam search is performed once on synthetic
proxy graphs and is not repeated for each target instance.

\textbf{Inference.} Once the best feature set is identified, the classifier is
fine-tuned on the target training graph and applied to the target instance in a single
forward pass at cost $O(|V| + |E|)$, matching the per-instance cost of QuickPrune.

\textbf{API cost.} Each full beam search run makes approximately 65 LLM calls (summary,
feature proposal, and code generation steps), totalling roughly 24,000 tokens per problem
type. This is a one-time cost amortized across all future instances of the same graph
family; per-instance inference requires no LLM calls.
...
(MaxCut), and Influence Maximization (IM), using
...
Chapters~3 and~4 are used, following~\citep{ireland2022lense}. For size constraints, we use Standard Greedy~\citep{nemhauser1978analysis} as the
downstream solver for MaxCover and MaxCut, and IMM~\citep{tang2015influence} for IM.
Under knapsack constraints, we use the budgeted maximum coverage
algorithm of~\citet{khuller1999budgeted} for MaxCover and IM, and the linear-time
algorithm of~\citet{pham2023linear} for MaxCut. The cost of each node under knapsack constraints is set proportional to its degree
...
All beam search results were generated using GPT-4.1 as the backbone LLM with temperature
$T = 1.0$. The beam search uses hyperparameters $D_{\max} = 5$, $\beta = 3$, $\gamma = 2$.
...
\caption{Comparison of pruning algorithms under size constraints. Reported are the pruning
  ...
  from~\citep{ireland2022lense, pmlr-v258-nath25a}. ``--'' indicates no reasonable result. Reprinted from~\citep{nath2026llmprune}.}
...
From Table~\ref{tab:llm2prune-size}, LLM2Prune performs competitively with QuickPrune
and SS on the combined metric while being orders of magnitude faster on large datasets
such as YouTube and Skitter (Figure~\ref{fig:llm2prune-runtime}). Among learning-based
methods, LLM2Prune consistently outperforms GCOMB-P, LeNSE, and COMBHelper in both
combined metric and runtime, without relying on hand-crafted features.
...
\begin{figure}[t]
  ...
  \caption{Runtime comparison (log scale) of pruning algorithms on Maximum Coverage,
    Maximum Cut, and Influence Maximization across eight datasets. Reprinted from~\citep{nath2026llmprune}.}
  \label{fig:llm2prune-runtime}
\end{figure}

\subsection{Discovered Features}
\label{sec:llm2prune-features}

A key strength of LLM2Prune is that it generates features that are not generic graph
statistics but are grounded in the problem objective and constraints.
Table~\ref{tab:llm2prune-features} summarizes the discovered features for all three
problems under both constraint types.

\begin{table}[t]
\centering
\caption{Features discovered by LLM2Prune for each CO problem and constraint type.
  All features were identified automatically from the task description with no human
  intervention. Reprinted from~\citep{nath2026llmprune}.}
\label{tab:llm2prune-features}
\small
\begin{tabular}{llp{7cm}}
\toprule
\textbf{Problem} & \textbf{Constraint} & \textbf{Discovered Features} \\
\midrule
MaxCover & Size     & Closed neighborhood size \\
         & Knapsack & Degree; coverage per cost \\
\midrule
MaxCut   & Size     & Degree; random cut expectation \\
         & Knapsack & Degree; weight; degree-to-weight ratio; normalized degree;
                      neighbor weight sum; clustering coefficient \\
\midrule
IM       & Size     & Degree; average incoming activation probability \\
         & Knapsack & Degree-to-weight ratio; outgoing activation probability sum;
                      in-out degree difference \\
\bottomrule
\end{tabular}
\end{table}

Under the size constraint, LLM2Prune discovers objective-grounded features across all
three problems. For MaxCover, it selects \emph{closed neighborhood size}, which directly
measures a node's local coverage potential. For MaxCut, it discovers \emph{random cut
expectation}, the expected marginal contribution of a node to the cut under a random
partition, a non-trivial descriptor derived from the cut objective. For IM, it identifies
\emph{average incoming activation probability}, which captures a node's susceptibility to
influence under the independent cascade model.

Under the knapsack constraint, LLM2Prune automatically encodes the cost-benefit trade-off
imposed by the constraint. For MaxCover it selects \emph{coverage per cost}, and for IM
it selects \emph{degree-to-weight ratio} and \emph{in--out degree difference}, capturing
cost-efficiency and directional propagation asymmetry respectively. For MaxCut under the
knapsack constraint, it discovers a richer set of six features including structural
properties such as clustering coefficient and neighbor weight sum. These features would
...
\subsection{Knapsack Constraint Results}

Under the knapsack constraint, only QuickPrune generalizes among prior methods; all
learning-based baselines are limited to size constraints and cannot be adapted without
significant redesign. LLM2Prune adapts by modifying the task description provided to the
LLM, with no changes to the underlying framework. We compare LLM2Prune against
QuickPrune~\citep{pmlr-v258-nath25a} and TOP-$k$, following~\citep{pmlr-v258-nath25a}.

Figure~\ref{fig:llm2prune-knapsack} shows that LLM2Prune outperforms TOP-$k$ across all
three problems. Compared to QuickPrune, LLM2Prune achieves higher combined metric on
MaxCover and IM, and remains competitive on MaxCut. These results demonstrate that
LLM2Prune generalizes naturally to different constraint types.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/llm2prune/algo_comparison_violin.pdf}
  \caption{Combined metric under knapsack constraint for LLM2Prune, QuickPrune, and
    TOP-$k$ across MaxCover, MaxCut, and IM. LLM2Prune outperforms TOP-$k$ on all
    problems and outperforms QuickPrune on MaxCov and IM. Reprinted from~\citep{nath2026llmprune}.}
  \label{fig:llm2prune-knapsack}
\end{figure}
...
problems under both size and knapsack constraints. The results show that beam search
consistently achieves higher and more stable performance across all six settings, with
median values close to $1.0$ and minimal variance across runs. Simpler variants suffer
from error propagation when early iterations contain reasoning errors, whereas beam search
mitigates this issue by maintaining multiple candidate paths. Notably, the advantage of
beam search is even more pronounced under knapsack constraints, where the feature space
is more complex.
...
\begin{figure}[t]
  ...
  \caption{Performance comparison of different variants of LLM2Prune. Beam search achieves
    ...
    variants. Reprinted from~\citep{nath2026llmprune}.}
