...
probability sum}, which require reasoning about the independent cascade model and capture
the susceptibility of a node and its direct influence spread, respectively. These features would
...
The primary remaining work includes: (1)~conducting a full comparative evaluation under
knapsack constraints against all baselines across all datasets and applications, and
(2)~evaluating LLM2Prune with open-source LLMs to assess whether the framework
generalizes beyond proprietary models.
Unlike other learning-based methods, LLM2Prune can naturally adapt to the knapsack
constraint by simply modifying the task description provided to the LLM, without requiring
any changes to the underlying framework or retraining the feature discovery pipeline.
