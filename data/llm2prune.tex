...
In this chapter, we present LLM2Prune~\cite{nath2025llm2prune}, a framework that uses
large language models (LLMs) to automatically generate node-level features for a
lightweight GNN classifier that predicts which nodes belong in the pruned ground set. The
feature discovery process is guided by an LLM-directed beam search over the feature space,
eliminating the need for manual feature engineering while maintaining strong performance
across different combinatorial optimization problems and constraint types. 
...
We consider the same pruning problem as in Chapters~3 and~4. Given a ground set
$\mathcal{U}$, a heuristic $\mathcal{H}$, and a constraint (either size or knapsack), the
goal is to construct a much smaller ground set $\mathcal{U}' \subseteq \mathcal{U}$ with
$|\mathcal{U}'| \ll |\mathcal{U}|$ such that solving the reduced instance yields nearly
the same objective value: $f(\mathcal{H}(\mathcal{U}')) / f(\mathcal{H}(\mathcal{U}))
\approx 1$. Unlike H-DeepPruner, which is restricted to size constraints, LLM2Prune
operates under both size and knapsack constraints by simply modifying the task description
provided to the LLM.

\section{Method}

The core idea of LLM2Prune is to use LLMs to automatically generate node-level features
for a lightweight GCN classifier~\cite{kipf2016semi} that predicts which nodes belong in
...
A classifier $g_\theta$ (a two-layer GCN~\cite{kipf2016semi}) is then trained on the
...
equally unimportant~\cite{manchanda2020gcomb}.

Feature-importance scores are then extracted using
GNNExplainer~\cite{ying2019gnnexplainer}, which provides attribution scores indicating
...
\FOR{$d = 0, 1, \dots, D_{\max} - 1$}
    ...
    \FOR{each $n_{d,i} \in B_d$} \hfill \textit{// Expansion}
...
We evaluate LLM2Prune on three graph CO problems, namely Maximum Coverage (MaxCover), Maximum
Cut (MaxCut), and Influence Maximization (IM)---under both size and knapsack constraints,
using eight real-world datasets from the Stanford Large Network Dataset
Collection~\cite{leskovec2016snap}. The same datasets and train-test splits as in
Chapters~3 and~4 are used, following~\cite{ireland2022lense}. For size constraints,
Standard Greedy~\cite{nemhauser1978analysis} is used for MaxCover and MaxCut, and
IMM~\cite{tang2015influence} for IM. Under knapsack constraints,
\cite{khuller1999budgeted} is used for IM and MaxCover, and \cite{pham2023linear} for
MaxCut. The cost of each node under knapsack constraints is set proportional to its degree
following~\cite{pmlr-v258-nath25a, yaroslavtsev2020bring}. Since running beam search on
each instance would be costly, we perform feature-space search using the Holme-Kim random
graph model~\cite{holme2002growing}, which closely resembles large-scale social networks
...
(QuickPrune~\cite{pmlr-v258-nath25a} and Submodular
Sparsification~\cite{zhou2017scaling}) and learning-based methods
(GCOMB-P~\cite{manchanda2020gcomb}, LeNSE~\cite{ireland2022lense}, and
COMBHelper~\cite{tian2024combhelper}). Table~\ref{tab:llm2prune-size} reports the results
...
\caption{Comparison of pruning algorithms under size constraints. Reported are the pruning
  ...
  from~\cite{ireland2022lense, pmlr-v258-nath25a}. ``--'' indicates no reasonable result.}
...
For the knapsack constraint, only QuickPrune among prior methods can be adapted. LLM2Prune
adapts to the knapsack constraint by simply modifying the task description provided to the
LLM. We also compare with TOP-$k$, a degree-based heuristic
following~\cite{pmlr-v258-nath25a}.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/llm2prune/algo_comparison_violin.pdf}
  \caption{Combined metric for LLM2Prune, QuickPrune, and TOP-$k$ under the knapsack
    constraint across MaxCover, MaxCut, and IM. LLM2Prune consistently outperforms TOP-$k$
    and matches or exceeds QuickPrune on MaxCover and IM.}
  \label{fig:llm2prune-knapsack}
\end{figure}

Figure~\ref{fig:llm2prune-knapsack} shows that LLM2Prune outperforms TOP-$k$ across all
applications and outperforms QuickPrune on MaxCover and IM, while being comparable on
MaxCut. Unlike other learning-based methods, LLM2Prune can easily adapt to the knapsack
constraint by simply modifying the task description, without any changes to the underlying
framework.

\subsection{Discovered Features}

A key strength of LLM2Prune is that it generates features that are not generic graph
statistics but are grounded in the problem objective and constraints. For MaxCut, it
discovers \emph{random cut expectation}, the expected marginal contribution of a node to
the cut under a random partition, a non-trivial descriptor directly derived from the cut
objective. For IM, it identifies \emph{average incoming activation probability} and
\emph{outgoing activation probability sum}, which require reasoning about the independent
cascade model and capture a node's susceptibility and direct influence spread,
respectively. Under the knapsack constraint, LLM2Prune automatically encodes the
cost-benefit trade-off imposed by the constraint: for MaxCover it selects \emph{coverage
per cost}, and for IM it selects \emph{degree-to-weight ratio} and \emph{in-out degree
difference}, capturing both cost-efficiency and directional propagation asymmetry. These
features would require significant domain expertise to design manually, yet LLM2Prune
discovers them automatically from the task description alone.

\subsection{Ablation Study}

To analyze the contribution of the major components of LLM2Prune, we evaluate four
...
problems under knapsack constraints. The results show that beam search consistently
achieves higher and more stable performance, with median values close to $1.0$ and minimal
variance across runs. Simpler variants suffer from error propagation when early iterations
contain reasoning errors, whereas beam search mitigates this issue by maintaining multiple
candidate paths.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/llm2prune/ablation.pdf}
  \caption{Performance comparison of different variants of LLM2Prune under knapsack
    constraints. Beam search achieves higher and more stable performance compared to
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/llm2prune/models.pdf}
  \caption{Comparison of LLM2Prune using two different LLMs. Both models achieve
    comparable performance across all applications and constraints.}
  \label{fig:llm2prune-models}
\end{figure}
