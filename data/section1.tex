...
Here we test the usage of references. The book\citep{REALCAR} 
is referred in this way. Actually, the option is available for you to change the default way how reference appears. The default and most commonly used option \citep{einstein} is displayed here \citep{Barn-JORVQ}.Both of these options are acceptable. 

Unrelated citations are referred here for the test of reference section only\citep{TAMU}. If you
find that the reference \citep{GIGEM} has more items than you need \citep{WAGFJ}, question marks will show up in place of a reference handle, like these \citep{Over9000}.
