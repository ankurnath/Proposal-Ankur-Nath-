...
\chapter{\uppercase {Related Work}} \label{chap:relatedwork}

This dissertation draws on five areas of prior research: the theory of submodular
...
limitations that motivate the contributions of this dissertation.
...
means. \citet{mirzasoleiman2015lazier} reduce the query complexity of greedy from
$O(nk)$ to $O(n\log(1/\epsilon))$ via stochastic lazy evaluation,
maintaining a $(1-1/e-\epsilon)$ guarantee. \citet{badanidiyuru2014streaming} introduce a single-pass
...
the TSP by adding nodes successively. \citet{joshi2019efficient} use a GCN to predict
edge inclusion probabilities for the TSP, decoded with beam search.
\citet{choo2022simulation} propose a fixed-width
...
propagation caused by simple feedback loops. A related line of work uses LLMs to directly generate algorithms or heuristics for
combinatorial problems. FunSearch~\citep{romera2024mathematical} applies an
LLM-guided evolutionary search to discover programs that solve problems such as bin
packing. \citet{yang2023large} use LLMs as black-box optimizers to iteratively refine
solution strategies via natural language feedback. \citet{liu2024evolution} evolve
heuristics for CO problems by prompting LLMs to propose and improve algorithmic rules.
While these methods share the idea of using LLM reasoning to search over a structured
space, their objective is orthogonal to LLM2Prune: they generate algorithms that are
re-run on each new instance, so their inference cost scales with graph size and degrades
on large instances. LLM2Prune instead uses LLMs only during a one-time feature
discovery phase on small synthetic graphs; inference on target instances reduces to a
single GCN forward pass, independent of graph size.
