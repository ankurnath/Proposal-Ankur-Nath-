...
\begin{frame}{Contributions}
  ...
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{\faCheckCircle\; LLM2Prune}
        \textit{TMLR 2026}~\cite{nath2026llmprune}\\[4pt]
...
\begin{frame}{Hierarchical DeepPruner: Results}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{figures/hdeeppruner/violin_plots.pdf}
...
\begin{frame}{LLM2Prune: Results (Size Constraint)}
  \begin{figure}
    \centering
...
\begin{frame}{LLM2Prune: Knapsack Constraint Results}
  Only QuickPrune generalizes to knapsack among prior methods; learning-based
  baselines cannot. LLM2Prune adapts by changing the prompt alone.

  \begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{Papers/LLM2Prune_TMLR/figures/algo_comparison_violin.pdf}
    \caption{Combined metric under knapsack constraint for LLM2Prune, QuickPrune, and
      TOP-$k$. LLM2Prune outperforms TOP-$k$ across all problems and outperforms
      QuickPrune on MaxCov and IM; comparable on MaxCut.}
  \end{figure}
\end{frame}

\begin{frame}{LLM2Prune: Discovered Features}
  LLM2Prune discovers features \textbf{grounded in the problem objective and constraints},
  not generic graph statistics:

  \vspace{0.3cm}

  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Maximum Cut (Knapsack)}
        \textbf{Degree-to-weight ratio}, \textbf{normalized degree},
        \textbf{neighbor weight sum}, and \textbf{clustering coefficient}:
        encode cost-efficiency and local structural properties relevant to the cut.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Influence Maximization (Knapsack)}
        \textbf{Degree-to-weight ratio}, \textbf{outgoing activation probability sum},
        and \textbf{in-out degree difference} capture cost-efficiency, direct
        influence spread, and directional propagation asymmetry.
      \end{block}
    \end{column}
  \end{columns}
  ...
  \vspace{0.3cm}
  \centering
  These require significant domain expertise to design manually, yet LLM2Prune
  discovers them \textbf{automatically from the task description alone}.
\end{frame}

\begin{frame}{LLM2Prune: Summary}
  \begin{itemize}
    \item Prunes $>90\%$ of the ground set while retaining near-optimal solution quality
    \item Competitive with classical methods (QuickPrune, SS); 1--4 orders of magnitude
      faster at inference on large graphs
    \item Consistently outperforms all learning-based baselines (GCOMB-P, LeNSE,
      COMBHelper) across 3 problems and 8 datasets
    \item Adapts to knapsack constraints via prompt change only,
      no algorithmic redesign required
    \item Automatically discovers problem-grounded features without any domain knowledge
  \end{itemize}
...
\begin{frame}{Conclusion}
  \textbf{Thesis:} Efficient pruning strategies can dramatically scale combinatorial
  optimization on graphs: with provable guarantees, without handcrafted features,
  and across diverse constraints.

  \vspace{0.3cm}

  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \begin{block}{QuickPrune}
        First algorithm with \textbf{provable} quality and size bounds. Pruned set
        size is logarithmic in $n$; handles size and knapsack constraints in a single
        pass.
      \end{block}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{H-DeepPruner}
        Demonstrates that \textbf{random features} suffice for learned pruning;
        no domain knowledge required. GNN + Neural MCTS captures element
        interdependencies.
      \end{block}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{LLM2Prune}
        \textbf{Automated feature discovery} via LLM-guided beam search. Adapts to
        new constraints by changing the prompt; 1--4 orders of magnitude faster than
        classical methods at inference.
      \end{block}
    \end{column}
  \end{columns}
  ...
  \vspace{0.3cm}
  \centering
  Together, these contributions form a coherent framework for scalable,
  general-purpose pruning of combinatorial optimization on graphs.
\end{frame}
...
\begin{frame}{QuickPrune: Why $\log(\kappa_{\max}/\kappa_{\min})$ Copies?}
  The budget range $[\kappa_{\min}, \kappa_{\max}]$ can be large. Instead of running
  a separate copy for every possible budget, we cover the range with
  \textbf{geometrically spaced checkpoints}:
  \[
    \kappa_{\max},\quad \kappa_{\max}(1-\eta),\quad \kappa_{\max}(1-\eta)^2,\quad \ldots,\quad \kappa_{\min}
  \]
  Each checkpoint is a factor $(1-\eta)$ smaller than the previous.
  The number of steps to go from $\kappa_{\max}$ down to $\kappa_{\min}$ is:
  \[
    i^* = \log_{1/(1-\eta)}\!\left(\frac{\kappa_{\max}}{\kappa_{\min}}\right)
    = \mathcal{O}\!\left(\log\frac{\kappa_{\max}}{\kappa_{\min}}\right)
  \]

  \begin{exampleblock}{Intuition}
    Just as halving a number repeatedly reaches $1$ in $\log_2 n$ steps, covering
    a budget range by shrinking geometrically takes logarithmically many checkpoints.
    We run one QuickPrune-Single copy per checkpoint, logarithmically many in
    total, and return their union.
  \end{exampleblock}
\end{frame}
...
\begin{frame}{QuickPrune: Time Complexity}
  \begin{block}{QuickPrune-Single}
    \begin{itemize}
      \item Each element is processed exactly \textbf{once} (streaming, single pass)
      \item Per-element cost: $\mathcal{O}(\log(\kappa_{\max}/\kappa_{\min}))$ oracle
        queries (one per parallel copy)
      \item Total oracle queries: $\mathcal{O}(n \log(\kappa_{\max}/\kappa_{\min}))$
      \item Space: $\mathcal{O}(|\mathcal{U}'|)$ to store the pruned set
    \end{itemize}
  \end{block}
...
\begin{frame}{H-DeepPruner: Time Complexity}
  \begin{block}{Stage 1: GNN Pre-Pruning}
    \begin{itemize}
      \item $d$-layer GCN over graph with $|V|$ nodes and $|E|$ edges:
        $\mathcal{O}(d(|V|+|E|))$ per forward pass
      \item Fixed $d = 2$ layers: $\mathcal{O}(|V|+|E|)$
      \item Runs \textbf{once} as a preprocessing step; no oracle queries at inference
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Stage 2: Neural MCTS Fine-Pruning}
    \begin{itemize}
      \item Operates on reduced graph with $|V'| \ll |V|$, $|E'| \ll |E|$
      \item Each simulation: depth $\ell$ GCN forward passes at cost
        $\mathcal{O}(\ell(|V'|+|E'|))$
      \item Progressive widening bounds branching factor: $\lfloor N^\alpha \rfloor$
        children for a node visited $N$ times ($\alpha \in (0,1)$)
      \item Progressive deepening halts at plateau: limits total simulations in practice
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}{H-DeepPruner: Speed-up Analysis}
  \begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{figures/hdeeppruner/speed_up.pdf}
    \caption{Downstream heuristic speed-up after pruning, $S = \text{time}_{\mathcal{U}} /
      (\text{time}_{\mathcal{U}'} + \text{time}_{\text{prune}})$, for Maximum Cover.}
  \end{figure}

  \vspace{0.2cm}

  \begin{alertblock}{Amortization}
    A single run's ratio is unfavorable: NMCTS pruning cost dominates the
    heuristic call it accelerates. The reported speed-ups (up to $40\times$) assume
    the pruned set $\mathcal{U}'$ is reused across many downstream runs, so that
    $\text{time}_{\text{prune}}$ is amortized over $r$ reuses:
    $S_r = \text{time}_{\mathcal{U}} / (\text{time}_{\mathcal{U}'} + \text{time}_{\text{prune}}/r)$.
  \end{alertblock}
\end{frame}

\begin{frame}{LLM2Prune: Runtime (Size Constraint)}
  \begin{itemize}
...
\begin{frame}{LLM2Prune: Time Complexity}
  \begin{block}{Beam Search (One-Time Cost)}
    \begin{itemize}
      \item Depth $D_{\max}$, beam width $\beta$, expansion factor $\gamma$:
        $\mathcal{O}(D_{\max} \cdot \beta \cdot \gamma)$ LLM calls total
      \item With fixed hyperparameters: \textbf{constant} number of API calls
      \item Per candidate: GCN training + GNNExplainer on synthetic graph
        $\mathcal{O}(|V_{\text{syn}}|+|E_{\text{syn}}|)$ per epoch
      \item Entire beam search run \textbf{once} on synthetic graphs; not repeated
        per target instance
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Inference (Per-Instance Cost)}
    \begin{itemize}
      \item Single GCN forward pass on target graph:
        $\mathcal{O}(|V|+|E|)$
      \item \textbf{No LLM calls at inference}; matches QuickPrune's per-instance cost
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]{LLM2Prune: Meta-Prompts (1/2)}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{$Z_{\text{summary}}$: History Summarization}
        \begin{minted}[fontsize=\tiny, breaklines]{text}
You are an expert in graph neural networks
and combinatorial optimization.

Summarize the following feedback from
previous iterations:
{cumulative_feedback}

Provide a concise summary of the key
points and insights.
        \end{minted}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{$Z_{\text{code}}$: Code Generation}
        \begin{minted}[fontsize=\tiny, breaklines]{text}
The input is a weighted NetworkX graph G
where each node has an attribute weight
...
extract_feature(G) that computes this
feature for ALL nodes in G. The function
must return a NumPy array with one value
per node, ordered to align with the
iteration order of G.nodes().
Keep the implementation efficient; avoid
...
\begin{frame}[fragile]{LLM2Prune: Meta-Prompts (2/2)}
  \begin{block}{$Z_{\text{feature}}$: Feature Generation}
    \begin{minted}[fontsize=\tiny, breaklines]{text}
...
Feedback from previous iterations: {explainer_feedback}
Refinement rules:
