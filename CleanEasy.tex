  ...
  \begin{columns}
    ...
    \begin{column}{0.42\textwidth}
      \begin{exampleblock}{Intuition}
        \textbf{Feasibility:} Elements exceeding the budget can never belong to
        any feasible solution --- discard immediately.

        \vspace{0.15cm}

        \textbf{Addition:} Retain only elements whose marginal value per unit cost
        is proportional to the current solution quality.

        \vspace{0.15cm}

        \textbf{Deletion:} When $f(A)$ grows by factor $n/\epsilon$ since the last
        checkpoint, the old elements' contributions are negligible by submodularity
        --- remove them in bulk to bound $|A|$.

        \vspace{0.15cm}

        \textbf{Singleton:} Guards against cases where no combination of elements
        outperforms a single high-value element.
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
