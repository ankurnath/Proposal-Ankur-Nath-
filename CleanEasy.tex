...
\begin{frame}{Related Work: Search Space Pruning}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{\faBook\; Classical \& Streaming}
        \begin{itemize}
          \item \citet{zhou2017scaling} (Submodular Sparsification): submodularity-based
            reduction; sequential, $O(n^2)$ pairwise evaluations; no quality guarantees
          \item \citet{badanidiyuru2014streaming} (Sieve-Streaming): single-pass streaming
            summarization via submodularity — targets data summarization, not CO solvers
          \item \citet{mirzasoleiman2020coresets}: coresets reduce training sets while
            preserving model quality — analogous goal, different problem setting
        \end{itemize}
      \end{block}

      \vspace{0.2cm}

      \begin{block}{\faGraduationCap\; Supervised (Problem-Specific)}
        \begin{itemize}
          \item \citet{lauri2019fine,lauri2023learning}: logistic regression +
            hand-crafted features for \textbf{Max Clique}
          \item \citet{fitzpatrick2021learning,sun2021generalization}: edge classifiers
            for \textbf{TSP}
          \item \citet{tian2024combhelper} (COMBHelper): GNN + knowledge distillation +
            problem-specific boosting
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{\faBrain\; Hybrid RL + Supervised}
        \begin{itemize}
          \item \citet{manchanda2020gcomb} (GCOMB): degree heuristic for initial pruning +
            Q-learning on pruned set
          \item \citet{ireland2022lense} (LeNSE): GNN embeddings + RL local search
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Three Open Problems in Search Space Pruning}
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \begin{alertblock}{\faShield\; No Guarantees}
    ...
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{alertblock}{\faCogs\; Handcrafted Features}
        Every learning-based pruner requires problem-specific feature engineering:
        degree heuristics, eigenvector centrality, Fourier mappings.

        \vspace{0.3cm}

        \textit{Can a pruner work without any domain knowledge?}
      \end{alertblock}
    \end{column}
    \begin{column}{0.32\textwidth}
