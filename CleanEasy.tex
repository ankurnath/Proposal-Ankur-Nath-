...
\begin{frame}{QuickPrune: Key Assumptions}
  ...
  \begin{block}{No-Heavy-Items (NHI) Assumption \small(needed for Theorem 2 only)}
    No single element alone consumes the entire budget --- each item in the
    optimal solution leaves some budget room ($\eta$-fraction) for others.

    \vspace{0.15cm}

    \begin{itemize}
      \item \textbf{Uniform costs} (size constraint): always satisfied --- each node
        costs $1$ out of a budget of $k \ge 2$.
      \item \textbf{Hub nodes in scale-free graphs:} a high-degree hub with cost
        $\approx \kappa$ would violate NHI. QuickPrune handles this via
        \textbf{singleton tracking} --- $a^*$ always retains the single best element
        as a fallback, so even extreme hubs are never discarded.
    \end{itemize}
  \end{block}
\end{frame}
