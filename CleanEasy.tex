...
\begin{frame}{QuickPrune: Key Assumptions}
  \begin{block}{$\gamma$-Weak Submodularity}
    ...
    Many real-world objectives (Influence Maximization, Maximum Cover) are fully submodular; MaxCut is
  ...
  \begin{block}{No-Heavy-Items (NHI) Assumption \small(needed for Theorem 2 only)}
    ...
    \begin{itemize}
      ...
      \item \textbf{Knapsack problems:} if a single element nearly exhausts the
        entire budget, the optimal solution is just that one element. A degenerate,
        uninteresting instance. Any meaningful multi-item selection problem
        naturally satisfies NHI.
...
\begin{frame}{LLM2Prune: Main Idea}
  \textbf{Insight:} Prior work has shown LLMs can automate feature engineering for
  tabular data~\cite{zhang2023openfe,han2024large,hollmann2023large}. We extend this
  to graph CO problems. 
  ...
  Unlike prior work, which derives features from existing ones
  using predefined rules or few-shot prompts and relies solely on performance feedback,
  LLM2Prune: (1) generates features from scratch with no predefined search space,
  (2) uses feature-importance scores (GNNExplainer) alongside performance to guide the LLM,
  and (3) employs beam search to reduce error propagation from simple feedback loops.
