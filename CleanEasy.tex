  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{\faBook\; Classical \& Streaming}
        \begin{itemize}
          \item Submodular Sparsification~\cite{zhou2017scaling}: submodularity-based
            reduction; sequential, $O(n^2)$ pairwise evaluations; no quality guarantees
          \item Sieve-Streaming~\cite{badanidiyuru2014streaming}: single-pass streaming
            summarization via submodularity --- targets data summarization, not CO solvers
          \item Coresets~\cite{mirzasoleiman2020coresets}: reduce training sets while
      ...
      \begin{block}{\faGraduationCap\; Supervised (Problem-Specific)}
        \begin{itemize}
          ...
          \item Fitzpatrick et al.~\cite{fitzpatrick2021learning,sun2021generalization}: edge classifiers
            for \textbf{TSP}
...
\begin{frame}{Three Open Problems in Search Space Pruning}
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \begin{alertblock}{\faShieldAlt\; No Guarantees}
