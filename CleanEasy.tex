...
\begin{frame}{Hierarchical DeepPruner: Limitations}
  ...
  \begin{enumerate}
    \item Size constraints only: the MDP formulation assumes a fixed number of
      elements; we have found that it does not work well for knapsack constraints

    \item MCTS scalability: progressive widening helps, but Stage 2 remains slow
...
\begin{frame}{LLM2Prune: Main Idea}
  ...
  How it works:
  \begin{enumerate}
    \item Give the LLM a text description of the CO problem and constraints
    \item The LLM proposes node-level features and writes Python code
      to compute each feature
    \item A lightweight GNN is trained on these features to classify nodes
    \item Feedback (which features helped, which didn't, and model performance) is sent back to the LLM
    \item The process repeats via \textbf{beam search} to find the best feature set
    \item Feature search is performed on small synthetic random graphs that
      approximate the target graph distributions, offsetting the cost of LLM queries;
      the resulting GCN is then fine-tuned on the target training graph
  ...
  \begin{exampleblock}{Key Advantage}
    ...
    constraints or to a new CO problem, just change the task description
    in the prompt.
  \end{exampleblock}
\end{frame}
