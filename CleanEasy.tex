...
\begin{frame}{Hierarchical DeepPruner: Stage 1, GNN Pre-Pruning}
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item 2-layer GCN for \textbf{binary classification}: is this node likely
          part of the optimal solution?
        \item \textbf{Input:} random node features $h_v \sim U(0,1)$;
          breaks symmetry without any domain knowledge
        \item \textbf{Training:} on small Erd\H{o}s-R\'{e}nyi graphs (2000 nodes),
          labels from a heuristic solver
        \item \textbf{Inference:} discard nodes predicted as non-solution in one pass
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{exampleblock}{Why Random Features?}
        ...
        that solution nodes share similar \textbf{local structural patterns},
...
\begin{frame}{Hierarchical DeepPruner: Stage 2, Neural MCTS Fine-Pruning}
  \textbf{Limitation of Stage 1:} GNN predicts node membership independently,
  potentially retaining redundant elements.

  \vspace{0.2cm}

  \textbf{Stage 2:} Formulate further pruning as an \textbf{MDP} solved with Neural MCTS.
  ...
  \begin{columns}
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item \textbf{State:} current pruned ground set + candidates
        \item \textbf{Action:} add an element to the pruned set
        \item \textbf{Reward:} heuristic objective value on pruned set
        \item \textbf{Terminal:} fixed number of elements added
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{MCTS with GNN guidance:}
      \begin{itemize}
        \item \textbf{Selection:} PUCT balances exploration vs.\ exploitation
        \item \textbf{Progressive widening:} gradually expands children ($\alpha = 0.1$)
        \item \textbf{Progressive deepening:} restarts if $\geq 1\%$ improvement;
  ...
  Builds the pruned set from scratch, capturing element \textbf{interdependencies}
...
\begin{frame}{Hierarchical DeepPruner: Limitations}
  ...
  \begin{enumerate}
    \item \textbf{Size constraints only:} the MDP formulation assumes a fixed number of
      elements; we have found out that it does not work well for knapsack constraint.

    \item \textbf{MCTS scalability:} Progressive widening helps, but Stage 2 remains slow
...
\begin{frame}{LLM2Prune: The Problem}
  ...
  \begin{itemize}
    ...
    \item \textbf{Learning-based methods} (GCOMB-P, LeNSE, COMBHelper):
      require \textbf{handcrafted features or design choices}; H-DeepPruner showed these
      are unnecessary but still needs \textbf{manual architecture/search decisions}
