...
\begin{frame}{QuickPrune: Key Assumptions}
  ...
  \begin{block}{No-Heavy-Items (NHI) Assumption \small(needed for Theorem 2 only)}
    For all $o$ in the optimal solution $O^\star$:
    \[
      c(o) \leq \kappa(1 - \eta), \quad \eta \in (0, 1/2].
    \]
    No single element's \textbf{cost} dominates the entire budget.

    \vspace{0.15cm}

    \textbf{When does it hold?}
    \begin{itemize}
      \item \textbf{Uniform costs} (size constraint): $c(o) = 1$, so NHI requires $\kappa \ge 2$ ---
        trivially satisfied for any non-trivial budget.
      \item \textbf{Knapsack / hub nodes:} If a high-degree hub has cost $\approx \kappa$,
        NHI may be tight (small $\eta$). In this case, the \textbf{singleton tracking} in
        QuickPrune handles it: $a^*$ always retains the single best element as a fallback.
    \end{itemize}
  \end{block}
\end{frame}
