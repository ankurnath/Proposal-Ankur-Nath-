...
\begin{frame}{Hierarchical DeepPruner: Stage 1, GNN Pre-Pruning}
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item GNN for \textbf{binary classification}: is this node likely
        ...
        \item \textbf{Training:} on small training graphs,
...
\begin{frame}{LLM2Prune: Motivation}
  \textbf{Key question:} Can we develop a pruning framework that \textbf{easily adapts
  to diverse constraints} and at the same time \textbf{does not require manual
  engineering} for each problem?

  \vspace{0.3cm}

  \textbf{Remaining gaps in learned pruning methods:}
  \begin{itemize}
    \item H-DeepPruner eliminates handcrafted features but is \textbf{limited to size constraints} and is slower than the classical QuickPrune algorithm
    \item GCOMB-P, LeNSE, and COMBHelper each require
      \textbf{problem-specific redesign} to handle new constraints or objectives
  ...
  \begin{block}{LLM2Prune}
    An LLM-directed beam search that \textbf{automatically discovers} pruning
...
\begin{frame}{LLM2Prune: Main Idea}
  ...
  \begin{enumerate}
    ...
    \item The LLM \textbf{proposes node-level features} and \textbf{writes Python code}
      to compute each feature
    \item A lightweight GNN is trained on these features to classify nodes
    \item \textbf{Feedback} (which features helped, which didn't, and model performance) is sent back to the LLM
    \item The process repeats via \textbf{beam search} to find the best feature set
    \item Feature search is performed on \textbf{small synthetic random graphs} that
      approximate the target graph distributions, offsetting the cost of LLM queries;
      the resulting GCN is then \textbf{fine-tuned on the target training graph}
      and evaluated on the test graph
  \end{enumerate}

  \vspace{0.2cm}

  \begin{exampleblock}{Key Advantage}
    Constraint-adaptive and problem-agnostic: to switch from size to knapsack
    constraints or to a new CO problem, just \textbf{change the task description
    in the prompt}. 
  \end{exampleblock}
\end{frame}
